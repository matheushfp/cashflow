...
public class ExpenseValidator : AbstractValidator<ExpenseRequest>
{
    public ExpenseValidator()
    {
        ...
        RuleFor(expense => expense.Date).LessThanOrEqualTo(DateTime.UtcNow).WithMessage(ResourceErrorMessages.EXPENSE_CANNOT_BE_IN_THE_FUTURE);
        ...
        {
            tag.IsInEnum().WithMessage(ResourceErrorMessages.INVALID_TAG);
        });
    }
}
